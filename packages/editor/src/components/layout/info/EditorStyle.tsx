...
const CanvasSettingsForm = () => {
  ...
  const [editorSize, setEditorSize] = useState(
    `${store.data.container.style.width} x ${store.data.container.style.height}`
  )

  // 画布背景
  const [elementBg, setElementBg] = useState(
    store.data.container.style.background || store.data.container.style.backgroundColor
  )
  ...
  return (
    <form style={{ padding: '0 10px' }}>
      <div className="el-row" style={{ gap: '10px' }}>
        <div className="el-col" style={{ width: '100px' }}>
          画布尺寸:
        </div>
      ...
      <div className="el-row" style={{ gap: '10px' }}>
        ...
        <div className="el-col" style={{ width: 'calc(100% - 100px)' }}>
          <InputNumber
            value={store.data.container.style.width}
            onChange={(val) => (store.data.container.style.width = val)}
          />
        </div>
      </div>
      ...
      <div className="el-row" style={{ gap: '10px' }}>
        ...
        <div className="el-col" style={{ width: 'calc(100% - 100px)' }}>
          <InputNumber
            value={store.data.container.style.height}
            onChange={(val) => (store.data.container.style.height = val)}
          />
        </div>
      </div>
      ...
      <div className="el-row" style={{ gap: '10px' }}>
        ...
        <div
          className="el-col"
          style={{ width: 'calc(100% - 100px)', display: 'flex', justifyContent: 'flex-end' }}
        >
          <input
            type="checkbox"
      ...
      {store.data.container.snapToGrid && (
        <>
          <div className="el-row" style={{ gap: '10px' }}>
            ...
            <div className="el-col" style={{ width: 'calc(100% - 100px)' }}>
              <InputNumber
                value={store.data.container.gridSize}
                onChange={(val) => (store.data.container.gridSize = val)}
              />
            </div>
          </div>
          ...
          <div className="el-row" style={{ gap: '10px' }}>
            ...
            <div className="el-col" style={{ width: 'calc(100% - 100px)' }}>
              <ColorPicker
                value={store.data.container.gridColor}
                onChange={(val) => (store.data.container.gridColor = val)}
              />
            </div>
          </div>
        </>
